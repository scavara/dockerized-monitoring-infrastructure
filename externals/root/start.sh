...
# ssh
iptables -A ssh -s X.X.X.X/32 -m comment --comment "p8s-db01" -j ACCEPT #master
#...
iptables -A ssh -m comment --comment "Drop all remaining traffic" -j REJECT --reject-with icmp-port-unreachable  

iptables -A crate -s X.X.X.X/32 -i eth0 -m comment --comment "access from master db node" -j ACCEPT
# ...
